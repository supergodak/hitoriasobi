...
const ENABLE_MAP_CLICK = false; // 地図タップ機能の無効化
...
const MapPage: React.FC = () => {
  ...
  const { locations, loading, error, createLocation } = useStores(bounds);
  ...
  const { markers, initialized } = useMarkerClusterer({
    map,
    locations: locations || [],
    onMarkerClick: (newSelectedLocations, position) => {
      if (tempMarker) {
        tempMarker.map = null;
        setTempMarker(null);
      }
  ...
  const handleMapClick = useCallback((e: google.maps.MapMouseEvent) => {
    if (!ENABLE_MAP_CLICK) return; // 地図タップ機能が無効な場合は早期リターン
    ...
    <div className="h-screen relative">
      ...
      {showSearchButton && (
        <button
          ...
          className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10 bg-green-600 text-white px-4 py-2 rounded-full shadow-md hover:bg-green-700 flex items-center"
        >
